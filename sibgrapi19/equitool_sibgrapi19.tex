...
We proposed the development of an equirectangular image quality assessment tool that uses objective metrics such as MSE, SSIM, and PSNR to facilitate choosing from different image resolutions and mapping solutions.

Our tool is integrated into the Unity Editor as Unity is the most used development engine for virtual reality applications. Different UV mapping techniques were used to visualize 360-degree images: latitude/longitude in an inverted sphere; skybox; and cubemap. We also demonstrated how to use an equirectangular image in a cubemap texture using a custom shader.

The Unity implementation assesses the image quality through rendering techniques for equirectangular images by generating viewport-based image patches. A python layer applies the full-reference objective metrics in each image patch using a correspondent patch from the skybox renderization.

One of the limitations of our tool is that the user needs to simulate the target device by manually setting a specific resolution and field view. For future work, we plan to obtain viewport patches directly from mobile devices where VR applications can be executed. We also plan an embedded component that allows reaping results while running the application on the mobile device. Additionally, we envision conducting subject evaluations to pick better metrics for quality assessment of image patches.
