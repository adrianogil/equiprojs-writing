...
% Geração de um cubo.
O primeiro passo do uso de um \textit{Cubemap} é a geração de um cubo. Porém o cubo padrão gerado pela Unity não possui vértices suficientes para um mapeamento de UV preciso. Para obter melhores resultados, utilizamos uma subdivisão de \textit{Mesh} em que cada triângulo é dividido em quatro. Iniciando de um cubo de $10$ vértices e $12$ triângulos, repetimos a operação de subdivisão quatro vezes e obtivemos um cubo de $4090$ vértices e $4096$ triângulos.

% Calculo de UV
No momento da geração de cada vértice, pode-se calcular a sua respectiva coordenada de UV, utilizando o cálculo da equação \ref{eq:uv_mapping_skybox}. Percebe-se que assim, a visão cubemap equivale a discretização da abordagem de mapeamento contínuo das coordenadas de UV em um \textit{Skybox}
